...
String meta_body(u32 magic, u32 abi, u32 tier, u32 max_pages)
{
    u8 raw[24];
    sys_put_u32(raw, magic);
    sys_put_u32(raw + 4, abi);
    sys_put_u32(raw + 8, tier);
    sys_put_u32(raw + 12, 0);
    sys_put_u32(raw + 16, 4);
    sys_put_u32(raw + 20, max_pages);
...
void test_sysabi()
{
    ...
    m.section(1, 4);
    m.custom("name", "xx");
    m.custom(PROC_SECTION, meta_body(PROC_MAGIC, PROC_ABI, 2, 256).str());
    m.section(10, 3);

    Result<ProcMeta> r = exec_meta(m.str());
    CHECK(r.is_ok());
    if (r.is_ok()) {
        CHECK_EQ(r.value().tier, 2);
