...
if (mode === "--kernel") {
    ...
    if (rootfs) {
        ...
        const RURL = "https://packages.test/braam";
        const serve = (stem, body) =>
            net.routes.set(`${RURL}/${stem}.zip`,
                           { status: 200, headers: "content-type: application/zip\n", body });
        const bytes = (path) => store.files.get(path);
        const text = (path) => {
            const b = bytes(path);
            if (!b)
        ...
        const good = archive("libz-1.0-r0");
        ...
        submit("rm -r /pkg/store /pkg/db", (ut += 0.005));
        prints("pkg install noisy",
    ...
    // Three copies of a 6,914-byte file: more than the eight writes a pipe
    ...
    // them.
    cshows("x=$(cat /home/c/big); echo \"$x\" | wc", "402 3216 20742");
