...
{
    Task<Result<Stat>> st = vfs_stat(e.path.str());
    if (!st)
    ...
    Task<Result<i32>> op = vfs_open(e.path.str(), O_READ);
    if (!op)
        co_return Err(Error::NoMemory);
    i32 fd = CO_TRY(co_await op);

    String text;
    u64 at = 0;
    for (;;) {
        u8 block[FS_BLOCK];
        Result<usize> r = vfs_read(fd, at, block, sizeof block);
        if (r.is_err()) {
            vfs_close(fd);
...
Task<Result<void>> save(Editor &e)
{
    ...
    Task<Result<i32>> op = vfs_open(e.path.str(), O_WRITE | O_CREATE | O_TRUNC);
    if (!op)
        co_return Err(Error::NoMemory);
    i32 fd = CO_TRY(co_await op);

    Str all = e.scratch.str();
    u64 at  = 0;
    while (at < all.size()) {
        usize n         = min(FS_BLOCK, all.size() - usize(at));
        Result<usize> w = vfs_write(fd, at, reinterpret_cast<const u8 *>(all.data() + at), n);
        if (w.is_err()) {
            vfs_close(fd);
...
void paint(Editor &e, const FullScreen &fs)
{
    Pane body = fs.body();
...
BRAAM_PROGRAM(prog_edit, "edit", "<file> — a full-screen editor; ^S saves, ^Q quits")
{
    if (args.size() != 2) {
        co_await io.err.write("usage: edit <file>\n");
        co_return 2;
    }

    Editor *e = heap_new<Editor>();
    if (!e) {
        co_await io.err.write("edit: out of memory\n");
        co_return 1;
    }
    struct Free {
        ~Free() { heap_delete(e); }
    ...
    KeyInput keys;
    if (!keys.ok()) {
        co_await io.err.write("edit: no keyboard\n");
        co_return 1;
    }

    FullScreen fs;
    if (!fs.ok()) {
        co_await io.err.write("edit: no screen\n");
        co_return 1;
    }
    screen_cursor(true);
