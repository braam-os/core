...
// service call, so `date` asks each time rather than caching an origin.
BRAAM_PROGRAM(prog_date, "date", "[-u] — print the date and time")
{
    bool utc = args.size() > 1 && args[1] == "-u";
    if (args.size() > 2 || (args.size() == 2 && !utc)) {
        co_await io.err.write("usage: date [-u]\n");
        co_return 2;
    }

    Result<WallClock> got = Err(Error::NoMemory);
    if (Task<Result<WallClock>> t = svc_clock())
        got = co_await t;
    if (got.is_err()) {
        if (got.error() == Error::Cancelled)
            co_return 130;
        co_await io.err.write("date: ");
