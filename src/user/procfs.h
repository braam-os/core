...
// line of state, so a directory level would hold exactly one file; BinFs made
// the same call for /bin. It lives in src/user/ for BinFs's reason as well —
// it reads the scheduler and the job table, and braam_fs must not depend
// upwards.
#pragma once
